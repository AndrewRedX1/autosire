...
func main() {
	...
	sunatClient := sunat.NewSunatClient(tokenService, 40*time.Second)
	...
	// Rutas de Ciclo de Vida de la Ventana
	var (
	...
	// 60 segundos de gracia inicial para que la ventana abra y cargue
	resetShutdownTimer(60 * time.Second)

	mux.HandleFunc("/api/heartbeat", func(w http.ResponseWriter, r *http.Request) {
		resetShutdownTimer(12 * time.Second)
		w.WriteHeader(http.StatusOK)
	})

	mux.HandleFunc("/api/shutdown", func(w http.ResponseWriter, r *http.Request) {
		resetShutdownTimer(2 * time.Second)
		w.WriteHeader(http.StatusOK)
	})
	...
	httpServer := &http.Server{
		Handler:           api.SecurityHeaders(api.SameOrigin(mux)),
